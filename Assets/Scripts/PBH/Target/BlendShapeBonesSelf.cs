...
/// <summary>
/// Unityのシステム上での、スキニングとブレンドシェイプの適用結果を取得するのが難しいので、自分で計算する
/// </summary>
public class BlendShapeBonesSelf
{
    ...
    public BlendShapeBonesSelf(SkinnedMeshRenderer smr, Transform myBone)
    {
        ...
        shapeCount = smr.sharedMesh.blendShapeCount;
    ...
    public ComputeBuffer Calculate(CommandBuffer cmd)
    {
        using (marker.Auto())
        {
            // ブレンドシェイプの加算結果をリセット
            // compute.Dispatch(kernelResetBlendShape, Mathf.CeilToInt(vertexCount / 64f), 1, 1);
            cmd.DispatchCompute(compute, kernelResetBlendShape, Mathf.CeilToInt(vertexCount / 64f), 1, 1);
            ...
            for (int i = 0; i < shapeCount; i++)
            {
                ...
                if (weight > 0)
                {
                    ...
                    cmd.SetComputeBufferParam(compute, kernelApplyBlendShape, "BlendShapeDeltas", blendShapeDeltasBuffers[i]);
                    cmd.SetComputeFloatParam(compute, "_BlendShapeWeight", weight);

                    // compute.Dispatch(kernelApplyBlendShape, Mathf.CeilToInt(vertexCount / 64f), 1, 1);
                    cmd.DispatchCompute(compute, kernelApplyBlendShape, Mathf.CeilToInt(vertexCount / 64f), 1, 1);
                }
            }
            ...
            cmd.SetComputeMatrixParam(compute, "_BoneMatrix", myBoneCurrentMatrix);

            // compute.Dispatch(kernelApplyBone, Mathf.CeilToInt(worldVerticesBuffer.count / 64f), 1, 1);
            cmd.DispatchCompute(compute, kernelApplyBone, Mathf.CeilToInt(worldVerticesBuffer.count / 64f), 1, 1);
        }
